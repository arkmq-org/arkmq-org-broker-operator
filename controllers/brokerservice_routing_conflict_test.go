...
func testMulticastRoutingForSubscriptions(t *testing.T, useShared bool) {
	...
	if eventsAddr.RoutingTypes != "MULTICAST" {
...
func testAnycastRoutingForConsumerOf(t *testing.T, useShared bool) {
	...
	if commandsAddr.RoutingTypes != "ANYCAST" {
...
func TestProcessCapabilities_ConflictingRoutingTypes_MultipleApps(t *testing.T) {
	...
	if addr := caps1.AddressConfigurations["shared-events"]; addr == nil || addr.RoutingTypes != "MULTICAST" {
	...
	if addr2 == nil || addr2.QueueConfigs["shared-events"] == nil || addr2.QueueConfigs["shared-events"].RoutingType != "ANYCAST" {
...
func TestProcessCapabilities_SharedAddress_BothSubscriptions(t *testing.T) {
	...
	if addr := caps1.AddressConfigurations["topic"]; addr == nil || addr.RoutingTypes != "MULTICAST" {
	...
	if addr := caps1.AddressConfigurations["topic"]; addr == nil || addr.QueueConfigs["sub1"] == nil || addr.QueueConfigs["sub1"].RoutingType != "MULTICAST" {
		t.Error("app1 should have MULTICAST queue sub1")
	}
	if addr := caps2.AddressConfigurations["topic"]; addr == nil || addr.QueueConfigs["sub2"] == nil || addr.QueueConfigs["sub2"].RoutingType != "MULTICAST" {
...
func TestProcessCapabilities_SharedAddress_BothConsumerOf(t *testing.T) {
	...
	if addr := caps1.AddressConfigurations["queue"]; addr == nil || addr.RoutingTypes != "ANYCAST" {
	...
	if addr := caps1.AddressConfigurations["queue"]; addr == nil || addr.QueueConfigs["queue"] == nil || addr.QueueConfigs["queue"].RoutingType != "ANYCAST" {
		t.Error("app1 should have ANYCAST queue")
	}
	if addr := caps2.AddressConfigurations["queue"]; addr == nil || addr.QueueConfigs["queue"] == nil || addr.QueueConfigs["queue"].RoutingType != "ANYCAST" {
