...
// processMonitoring generates what a Prometheus needs to scrape this app's own
...
// namespaces. The Probe lives in the app's namespace, so its series carry that
// namespace, and whoever may read metrics there sees this app's queues only.
func (reconciler BrokerAppInstanceReconciler) processMonitoring() {
	...
	if caRef == nil {
		reconciler.log.V(1).Info("no ca bundle, generating no Probe", "app", reconciler.instance.Name)
		return
	}
	...
	reconciler.TrackDesired(monitoring.BuildProbe(monitoring.ScrapeTarget{
		...
		// Probe is written the exact pod to scrape is already known.
		Host: common.OrdinalFQDNS(service.Name, service.Namespace, 0),
		Port: monitoring.Port,
		// see the note in the BrokerService's processMonitoring: only the wildcard
		// SAN covers a fully qualified pod name
