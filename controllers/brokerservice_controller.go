...
func (reconciler *BrokerServiceInstanceReconciler) processCapabilities(secret *corev1.Secret, app *broker.BrokerApp) (err error) {
	...
	ensureAddrCfg := func(addressKey string) *brokerproperties.AddressConfiguration {
		if cfg.AddressConfigurations[addressKey] == nil {
			cfg.AddressConfigurations[addressKey] = &brokerproperties.AddressConfiguration{}
		}
		return cfg.AddressConfigurations[addressKey]
	}

	ensureSecRole := func(addressKey, roleKey string) *brokerproperties.RolePermissions {
		if cfg.SecurityRoles[addressKey] == nil {
			cfg.SecurityRoles[addressKey] = map[string]*brokerproperties.RolePermissions{}
		}
		if cfg.SecurityRoles[addressKey][roleKey] == nil {
			cfg.SecurityRoles[addressKey][roleKey] = &brokerproperties.RolePermissions{}
		}
		return cfg.SecurityRoles[addressKey][roleKey]
	}
	...
	for addressName, addr := range addressTracker.names {
		...
		if !addr.isMulticast {
			if addrCfg.QueueConfigs == nil {
				addrCfg.QueueConfigs = map[string]*brokerproperties.QueueConfig{}
			}
			addrCfg.QueueConfigs[queueKey] = &brokerproperties.QueueConfig{RoutingType: "ANYCAST", Address: addressKey}
			queueNamesForMetrics[queueKey] = true
		}
		...
		if isFQQN {
			...
			addrCfg.QueueConfigs[queueKey] = &brokerproperties.QueueConfig{RoutingType: "MULTICAST", Address: addressKey}
			queueNamesForMetrics[queueKey] = true
		}

		// Always generate RBAC roles (for both owned and referenced addresses)
		for _, role := range addr.senderRoles {
			ensureSecRole(addressName, producerRole(role)).Send = true
		}
		for _, role := range addr.consumerRoles {
			ensureSecRole(addressName, consumerRole(role)).Consume = true
		}
	...
	for queueName := range queueNamesForMetrics {
		for _, rbacRole := range []string{"metrics", metricsRole(AppIdentity(app))} {
			ensureSecRole("mops.queue."+queueName, rbacRole).View = true
			ensureSecRole("mops.queue."+queueName+".getMessageCount", rbacRole).View = true
			ensureSecRole("mops.queue."+queueName+".getConsumerCount", rbacRole).View = true
			ensureSecRole("mops.queue."+queueName+".getDeliveringCount", rbacRole).View = true
			ensureSecRole("mops.queue."+queueName+".getPersistentSize", rbacRole).View = true
		}
	}

	// queryMBeans only enumerates; the per-queue grants above decide what comes back
	if len(queueNamesForMetrics) > 0 {
		ensureSecRole("mops.mbeanserver.queryMBeans", metricsRole(AppIdentity(app))).View = true
	}
			...
			realmName: {
				Modules: brokerproperties.JaasModules{
					Cert: brokerproperties.JaasLoginModule{
						LoginModuleClass: "org.apache.activemq.artemis.spi.core.security.jaas.TextFileCertificateLoginModule",
