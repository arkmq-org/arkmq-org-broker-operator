...
//
// Silently generates nothing when the ScrapeConfig kind is not served or the
// feature is switched off; the BrokerService is perfectly functional without it.
func (reconciler *BrokerServiceInstanceReconciler) processMonitoring() {
	...
	if _, err := common.GetNamespacedSecret(reconciler.Client, promCertSecret, instance.Namespace); err != nil {
		reconciler.log.V(1).Info("no prometheus cert, generating no ScrapeConfig",
	...
	if caRef == nil {
		reconciler.log.V(1).Info("no ca bundle, generating no ScrapeConfig", "service", instance.Name)
		return
	}
