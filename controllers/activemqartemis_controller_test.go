...
var _ = Describe("artemis controller", func() {
	...
	Context("tls secret reuse", Label("tls-secret-reuse"), func() {
		It("console and acceptor share one secret", func() {
			if os.Getenv("USE_EXISTING_CLUSTER") == "true" {
				...
				brokerCr, createdBrokerCr := DeployCustomBrokerV1(defaultNamespace, func(candidate *brokerv1beta2.BrokerCluster) {
		...
		It("exposes acceptors and console via Gateway API routes", func() {
			...
			brokerCr, createdBrokerCr := DeployCustomBrokerV1(defaultNamespace, func(candidate *brokerv1beta2.BrokerCluster) {
				...
				gatewayHostTemplate := "$(ITEM_NAME)-$(BROKER_ORDINAL).$(INGRESS_DOMAIN)"

				candidate.Spec.Console.Expose = true
				candidate.Spec.Console.SSLEnabled = true
				candidate.Spec.Console.SSLSecret = commonSecretName
				candidate.Spec.Console.ExposeMode = &gatewayMode
				candidate.Spec.Console.IngressHost = gatewayHostTemplate
				...
				candidate.Spec.Acceptors = []brokerv1beta2.AcceptorType{
					{
						...
						IngressHost: gatewayHostTemplate,
			...
			By("verifying the broker reconciles cleanly, i.e. the gateway routes are tracked as deployed")
			// without the routes in GetDeployedResources every reconcile retries a
			// create and fails with AlreadyExists, so the CR never becomes Ready
			Eventually(func(g Gomega) {
				g.Expect(k8sClient.Get(ctx, types.NamespacedName{Name: brokerCr.Name, Namespace: defaultNamespace}, createdBrokerCr)).To(Succeed())
				g.Expect(meta.IsStatusConditionTrue(createdBrokerCr.Status.Conditions, brokerv1beta2.ValidConditionType)).Should(BeTrue())
				g.Expect(meta.IsStatusConditionTrue(createdBrokerCr.Status.Conditions, brokerv1beta2.ReadyConditionType)).Should(BeTrue())
			}, existingClusterTimeout, existingClusterInterval).Should(Succeed())
	...
	Context("Validation", func() {
		It("with test labels", func() {
			...
			validBrokerClusterCR, convertErr := ConvertArtemisToBrokerCluster(&validCrd)
			Expect(convertErr).To(BeNil())
			deployedResources, err = common.GetDeployedResources(validBrokerClusterCR, k8sClient, isOpenshift, isGatewayAPIAvailable)
			...
			invalidBrokerClusterCR, convertErr := ConvertArtemisToBrokerCluster(&invalidCrd)
			Expect(convertErr).To(BeNil())
			deployedResources, err = common.GetDeployedResources(invalidBrokerClusterCR, k8sClient, isOpenshift, isGatewayAPIAvailable)
		...
		It("with ingress in openshift", Label("with-ing-in-openshift"), func() {
			...
			Eventually(func(g Gomega) {
				...
				deployedBrokerClusterCR, convertErr := ConvertArtemisToBrokerCluster(deployed)
				g.Expect(convertErr).To(BeNil())
				deployedResources, err := common.GetDeployedResources(deployedBrokerClusterCR, k8sClient, true, isGatewayAPIAvailable)
