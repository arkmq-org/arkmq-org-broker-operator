...
var _ = Describe("artemis controller", func() {
	...
	Context("Console secret Test", func() {
		...
		It("deploy broker with ssl enabled console", func() {
			...
			outer := NewBrokerReconciler(k8Manager, ctrl.Log, isOpenshift, isGatewayAPIAvailable)
	...
	Context("Validation", func() {
		It("with test labels", func() {
			...
			Expect(convertErr).To(BeNil())
			deployedResources, err = common.GetDeployedResources(validBrokerCR, k8sClient, isOpenshift, isGatewayAPIAvailable)
			...
			Expect(convertErr).To(BeNil())
			deployedResources, err = common.GetDeployedResources(invalidBrokerCR, k8sClient, isOpenshift, isGatewayAPIAvailable)
		...
		It("with ingress in openshift", Label("with-ing-in-openshift"), func() {
			...
			Eventually(func(g Gomega) {
				...
				deployedResources, err := common.GetDeployedResources(deployedBrokerCR, k8sClient, true, isGatewayAPIAvailable)
