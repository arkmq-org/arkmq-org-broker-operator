...
// Probe kind. It says nothing about a Prometheus actually running: that
...
// probeIsServed treats every failure as "not served": a missing CRD is the
...
func probeIsServed(mapper meta.RESTMapper) bool {
	if mapper == nil {
		return false
	}
	...
	if _, err := mapper.RESTMapping(groupKind, monitoringv1.SchemeGroupVersion.Version); err != nil {
		if !meta.IsNoMatchError(err) {
			ctrl.Log.V(1).Info("could not determine whether Probe is served", "err", err)
		}
		return false
	}
