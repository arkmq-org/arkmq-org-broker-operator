...
flowchart TD
    ...
    PROC["Order Processor<br/><code>order-processor-app</code>"]
    Q2[("Queue<br/><b>ORDERS.PROCESSED</b>")]
    SHIP["Shipping Service<br/><code>shipping-service-app</code>"]
    Q3[("Queue<br/><b>ORDERS.SHIPPED</b>")]
    DELIV["Delivery Service<br/><code>delivery-service-app</code>"]
    Q4[("Queue<br/><b>ORDERS.DELIVERED</b>")]
    SINK["Master Sink<br/><i>optional drain</i>"]
...
kubectl wait clusterissuer root-issuer --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
kubectl wait clusterissuer broker-ca-issuer --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
kubectl wait certificate arkmq-org-broker-manager-cert -n service-app-project --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
kubectl wait certificate messaging-service-broker-cert -n service-app-project --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
kubectl wait BrokerService messaging-service -n service-app-project --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
kubectl wait certificate order-generator-app-cert -n service-app-project --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
kubectl wait BrokerApp order-generator -n service-app-project --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
kubectl wait certificate order-processor-app-cert -n service-app-project --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
kubectl wait BrokerApp order-processor-app -n service-app-project --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
kubectl wait certificate shipping-service-app-cert -n service-app-project --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
kubectl wait BrokerApp shipping-service-app -n service-app-project --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
kubectl wait certificate delivery-service-app-cert -n service-app-project --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
kubectl wait BrokerApp delivery-service-app -n service-app-project --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
order-processor-app-binding-secret                                                    Opaque                                3      21m
...
kubectl wait certificate master-sink-app-cert -n service-app-project --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
kubectl wait BrokerApp master-sink-app -n service-app-project --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
kubectl wait certificate prometheus-cert -n service-app-project --for=condition=Ready --timeout=600s
```
```shell markdown_runner
...
kubectl wait pod/messaging-service-ss-0 -n service-app-project --for=condition=Ready --timeout=600s
