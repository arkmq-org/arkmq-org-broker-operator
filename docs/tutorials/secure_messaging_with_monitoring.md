...
graph TD
    subgraph broker_tutorial ["broker-tutorial namespace"]
        ...
        subgraph monitoring ["Monitoring Stack"]
            SvcScrape["Probe<br/>(messaging-service-metrics)"]
            AppScrape["Probe<br/>(first-app-metrics)"]
...
The `probeSelectorNilUsesHelmValues=false` flag is what lets this Prometheus
pick up the scrape wiring the operator generates. By default the chart restricts
it to objects labelled with its own Helm release, which would exclude them.
...
the operator has already generated everything needed to scrape it: a `Probe`
for the service, and another for the app. The Prometheus installed by
`kube-prometheus-stack` picks them up on its own, because of the selector flag
set at [install time](#install-prometheus-operator) — there is nothing to deploy
here and nothing to parametrize per service or per app.

### Inspect what the operator generated

```bash {"stage":"scrape", "runtime":"bash", "label":"show generated scrape wiring"}
kubectl get probe -n broker-tutorial -l broker.arkmq.org/monitoring=true
```
```shell markdown_runner
NAME                        AGE
...
and that it names the broker pod:
```bash
kubectl get probe -n broker-tutorial -l broker.arkmq.org/monitoring=true
```
...
* The operator generates the `Probe` objects that enable secure metrics
...
* **Operator generated scrape config:** both `Probe` objects are created by
